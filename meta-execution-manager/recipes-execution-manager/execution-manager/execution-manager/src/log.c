...
**  This work was derived from https://github.com/containers/crun/blob/1.4.3/src/libcrun/error.c and
**  https://github.com/containers/crun/blob/1.4.3/src/libcrun/utils.c
**/
...
#define MAKE_ERROR(FUNC_NAME)                                            \
  int FUNC_NAME (log_error_t *err, int status, const char *msg, ...) \
  {                                                                      \
    va_list args_list;                                                   \
    log_error_t ptr;                                                 \
    va_start (args_list, msg);                                           \
    *err = xmalloc (sizeof (struct log_error_s));                    \
    ptr = *err;                                                          \
    ptr->status = status;                                                \
...
MAKE_ERROR (execution_manager_make_error);

int
execution_manager_error_wrap (log_error_t *err, const char *fmt, ...)
{
...
int
execution_manager_error_release (log_error_t *err)
{
  log_error_t ptr;
  if (err == NULL)
    return 0;
...
int
log_error_release (log_error_t *err)
{
  return execution_manager_error_release (err);
}

void
execution_manager_error_write_warning_and_release (FILE *out, log_error_t **err)
{
  log_error_t ref;
...
void
log_error_write_warning_and_release (FILE *out, log_error_t **err)
{
  return execution_manager_error_write_warning_and_release (out, err);
}

int
execution_manager_error_get_errno (log_error_t *err)
{
  if (err == NULL || *err == NULL)
    return 0;
  return (*err)->status;
...
int
execution_manager_init_logging (execution_manager_output_handler *new_output_handler, void **new_output_handler_arg, const char *id,
                      const char *log, log_error_t *err)
{
  if (log == NULL)
  ...
  else
    {
      ...
      if (log_type < 0)
        return execution_manager_make_error (err, errno, "unknown log type %s\n", log);
        ...
        case LOG_TYPE_FILE:
          ...
          *new_output_handler_arg = fopen (arg, "a+");
          if (*new_output_handler_arg == NULL)
            return execution_manager_make_error (err, errno, "open log file %s\n", log);
          break;
  ...
  execution_manager_set_output_handler (*new_output_handler, *new_output_handler_arg, log != NULL);
  return 0;
}
...
static execution_manager_output_handler output_handler = log_write_to_stderr;
static void *output_handler_arg = NULL;
static int output_verbosity = LOG_VERBOSITY_ERROR;

void
execution_manager_set_verbosity (int verbosity)
{
  output_verbosity = verbosity;
}

int
execution_manager_get_verbosity ()
{
  return output_verbosity;
}

void
execution_manager_set_output_handler (execution_manager_output_handler handler, void *arg, bool log_to_stderr)
{
...
write_log (int errno_, bool warning, const char *msg, va_list args_list)
{
  ...
  if (warning && output_verbosity < LOG_VERBOSITY_WARNING)
    return;
...
void
execution_manager_warning (const char *msg, ...)
{
  va_list args_list;
  va_start (args_list, msg);
...
void
log_error (int errno_, const char *msg, ...)
{
  va_list args_list;
  va_start (args_list, msg);
...
void __attribute__ ((noreturn)) execution_manager_fail_with_error (int errno_, const char *msg, ...)
{
  va_list args_list;
  va_start (args_list, msg);
  write_log (errno_, false, msg, args_list);
...
int
execution_manager_set_log_format (const char *format, log_error_t *err)
{
  if (strcmp (format, "text") == 0)
    ...
    return execution_manager_make_error (err, 0, "unknown log format type %s", format);
